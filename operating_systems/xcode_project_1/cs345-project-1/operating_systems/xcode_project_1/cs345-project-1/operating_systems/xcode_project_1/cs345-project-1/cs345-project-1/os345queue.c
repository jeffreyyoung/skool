...
PriorityQueue* initQueue(PriorityQueue* pq)
{
    pq = (PriorityQueue*)malloc(sizeof(PriorityQueue));	//Changed after causing immense pain in lab 5
    //not enough memory :()
    pq->queue_tasks = 0;
    return pq;
}
...
task enQ(PriorityQueue* pq, int t_id, int prior)
{
    task to_add;
    to_add.tid = t_id;
...
void printPQ(PriorityQueue* pq)
{
    ...
    {
        printf("\nPQ[%d] : tid: %d    priority: %d  ", i,pq->queue[i].tid, pq->queue[i].priority);
    }
}